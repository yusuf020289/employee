...
@RestController
@RequestMapping(EmployeeRestConstant.EMP_ADD)
public class EmployeeAddController extends AbstractPostController<EmployeeAddRequest> {
...
private static Logger LOGGER = LoggerFactory.getLogger(EmployeeInquiryController.class);
	...
	@Override
	public EmployeeAddRequest validatePost(EmployeeAddRequest request) throws ApplicationException {
		...
		try {
			if (errors.isEmpty()) {
				if (employeeService.isExistEmployeeByNamaAndTanggal(request.getNama(), request.getTanggal(), null)) {
	...
	@Override
	public ResponseEntity<Response> executePost(EmployeeAddRequest request) throws Exception {
	...
	private Employee mapRequestToEntity(EmployeeAddRequest request) throws Exception {
	...
	public void constructAuditSuccess(RequestMethod method,
			EmployeeAddRequest request, ResponseEntity<Response> response)
			throws Exception {
	}

	@Override
	public void constructAuditError(RequestMethod method,
			EmployeeAddRequest request, List<ErrorHolder> errors) throws Exception {
	}
